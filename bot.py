...
async def accounts_page(update, context):
    ...
    if not sessions:
        lines.append("❌ Koi account login nahi hai.")
    else:
        for slot, phone, session_string, account_name, stopped in sessions:
            status = "🛑 Stopped" if stopped else "🟢 Active"
            name = account_name or "Unknown"
            mark = " ⭐" if slot == active_slot else ""
            lines.append(
                f"**Slot {slot}**{mark}\n"
                f"👤 {name}\n"
                f"📱 {phone or 'N/A'}\n"
...
async def login_page(update, context):
    ...
    sessions = get_user_sessions(user_id)
    used_slots = {row[0] for row in sessions}
    free_slot = next(
...
def main_keyboard(user_id):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "🔐 Login Account", callback_data="login_account"
            ),
            InlineKeyboardButton(
        ...
        [
            ...
            InlineKeyboardButton("🛠️ Help Centre", callback_data="help_centre"),
        ],
    ]
...
async def settings_page(update, context):
    await update.callback_query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton(
                "📢 Source Channel", callback_data="opt_1"
            )],
            [InlineKeyboardButton(
                "👥 Target Groups", callback_data="opt_2"
            )],
            [InlineKeyboardButton(
                "⏱️ Time Interval", callback_data="opt_3"
            )],
            [InlineKeyboardButton(
                "💬 Share Message", callback_data="opt_4"
            )],
            [InlineKeyboardButton(
                "🔙 Back to Menu", callback_data="main_menu"
            )],
...
async def source_channel_page(update, context):
    ...
    keyboard = [
        [InlineKeyboardButton(
            f"📌 {name}", callback_data=f"channel_{index}"
        )]
...
async def admin_command(update, context):
    ...
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "💎 Manage Subscription", callback_data="admin_sub"
            )],
            [InlineKeyboardButton(
                "📊 User Stats", callback_data="admin_stats"
            )],
            [InlineKeyboardButton(
...
async def button_handler(update, context):
    ...
    if data.startswith("channel_"):
        try:
            ...
            # Store numeric ID. This is more reliable than title lookup.
    ...
    if (
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
                        "💎 ₹399 — 30 Days", callback_data="plan_30"
                    )],
                    [InlineKeyboardButton(
                        "💎 ₹799 — 90 Days", callback_data="plan_90"
                    )],
                    [InlineKeyboardButton(
                        "💎 ₹1999 — 180 Days", callback_data="plan_180"
                    )],
                    [InlineKeyboardButton(
                        "🔙 Cancel", callback_data="main_menu"
                    )],
...
async def start_background_workers(application):
    ...
    try:
        ...
        tasks.append(
            asyncio.create_task(
                background_forwarder(application)
            )
        )
    ...
    try:
        ...
        tasks.append(
            asyncio.create_task(
                expiry_reminder_worker(application)
            )
        )
